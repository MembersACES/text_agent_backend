...
def drive_filing(file_bytes: bytes, filename: str, business_name: str, gdrive_url: str, filing_type: str) -> dict:
    ...
    Args:
        ...
        filing_type (str): Type of filing (loa, savings, revenue, site_profiling)
    Returns:
    ...
    try:
        ...
        new_filename = f"{business_name} - {display_name}.{filename.split('.')[-1]}"
        files = {
            'file': (new_filename, file_bytes, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        }
        data = {
        ...
        if response.status_code == 200:
            return {"status": "success", "message": f"Successfully processed {display_name} filing for {business_name}", "filename": new_filename}
        else:
            return {"status": "error", "message": f"Failed to process {display_name} filing. Status code: {response.status_code}, Response: {response.text}"}
